...
//
//            case 0:
        $msg = "【".$this->sms_header."】老板你好，我是".$sender."，我们给您提供了店铺".$bizName."的装修方案，详情请进入".$this->sms_header."商户版查收，如有疑问请联系：".$stel."。";
        $result_code = $this->smsChinaSend($tel,$msg);
        return $result_code;
...
//                     $result = $this->ytxREST->sendTemplateSMS($to, array($username, $password, $app_down_url), "137889");
                    ...
                    $result = $this->ytxREST->sendTemplateSMS($to, array($username, $password, $url), "203161");
                } else if ($this->is_qqcg == 1) {
                    $result = $this->ytxREST->sendTemplateSMS($to, array($username, $password, $url), "203162");
                } else {
                    $msg = "【".$this->sms_header."】您的店铺已开通,账号:" . $username . ",密码:" . $password . ",点击链接查看开通信息:".$url;
                    $result_code = $this->smsChinaSend($to, $msg);
                    return $result_code;
                }
                if ($result == NULL) {
...
//            }
	...
	/*发送短信
	 * $params tinyint $msg_type 消息类型 1-验证码类消息 2-通知类消息
	*/
	public function sendSms($msg_type = 1, $msg = ''){
