...
class CartLogic extends BaseLogic
{
    ...
    public function __construct($storeId = 0, $memberId = 0, $name = '', $tablePrefix = '', $connection = '')
    {
        ...
        $cart = D('Cart')->getCart($this->cartStoreId, $this->memberId);
    ...
    public function modNum($goods_id, $spec_id, $num, $otherData = [])
    {
        ...
        if ($num <= 0) {
            ...
            if ($this->storeGrant['group_buy_ctrl'] == 1 && $this->storeInfo['is_force_group'] == 1) {
                $result = D('GroupBuy')->checkGoodsInGroup($goods_id);
                if (!empty($result['msg'])) {
                    unset($this->items[$goods_id . '|' . $spec_id]);
                    $this->saveItem();
                    return '该商品已加入团购,无法直接购买';
                }
            }
            $buy_history = 0;
    ...
    public function saveItem()
    {
        ...
        if (!empty($this->cartStoreId) && !empty($this->memberId)) {
            ...
            $shop_cart_data = M('mb_shop_cart')->where(array('member_id' => $this->memberId, 'store_id' => $this->cartStoreId))->find();
            $data = [];
            ...
            if (!empty($shop_cart_data)) {
                ...
                if ($old != $data['serialized_data']) {
                    ...
                    $result = M('mb_shop_cart')->where(array('member_id' => $this->memberId, 'store_id' => $this->cartStoreId))->save($data);
                    // 更新后同步旧数据 下次比较才是正确的
                    if ($result !== false) {
                        $this->oldItems = $this->items;
                    }
                ...
                return M('mb_shop_cart')->add($data);
            }
        }
    }
     ...
     * @return boolean
     * User: hjun
     * Date: 2019-03-14 17:24:45
     * Update: 2019-03-14 17:24:45
     * Version: 1.00
     */
    public function isGsIdInvalid($gsId = '')
    {
        $ids = explode("|", $gsId);
        $specStr = $ids[1];
        $delete = M('goods')->where(array('goods_id' => $ids[0]))->field('isdelete,goods_state,spec_open,goods_spec')->find();
        // 已删除 则无效
        if ($delete['isdelete'] != 0) {
            return true;
        }
        // 已下架 则无效
        if ($delete['goods_state'] != 1) {
            return true;
        }
        // 获取规格
        $goodsSpec = jsonDecodeToArr($delete['goods_spec']);
        $specOption = [];
    ...
    public function onceAgain($items)
    {
        ...
        foreach ($items as $item) {
            $gsId = explode('|', $item['gs_id']);
            if ($this->isSpecIdEmpty($gsId[1])) {
                $gsId[1] = 0;
                $oldNum = $this->getNumWithID("{$gsId[0]}|{$gsId[1]}");
                $result = $this->addItem($gsId[0], 0, $item['gou_num'] + $oldNum);
            } else {
                $oldNum = $this->getNumWithID("{$gsId[0]}|{$gsId[1]}");
                $result = $this->addItem($gsId[0], $gsId[1], $item['gou_num'] + $oldNum);
            }
            if (!empty($result)) {
                $error = [];
     ...
     * Version: 1.00
     */
    public function getShopCartGoodsList($gsIds = [])
    {
        $items = $this->items;
        // 如果有指定商品 则只获取指定的商品
        if (!empty($gsIds)) {
    ...
    public function initGoodsBeans($goodsBeans, $type = 0, $initType = self::INIT_TYPE_ALL)
    {
        ...
        for ($i = 0; $i < count($goodsBeans); $i++) {
            ...
            // hjun
            $goodsBeans[$i]['buy_num'] = $this->getBuyNumByGoodsId($goodsBeans[$i]['goods_id']);
            ...
            if ($goodsBeans[$i]['spec_open'] == 0) {    //未开启新版规格
                ...
                if (!empty($sale) && $sale['islongtime'] == 1 && $sale['isdelete'] == 0) {
                    ...
                    $goodsBeans[$i]['goods_pv'] = $goodsBeans[$i]['new_price'] - ($goodsBeans[$i]['goods_price'] - $goodsBeans[$i]['goods_pv']);
                } else {
                    /*  */
...
//                    $goodsBeans[$i]['buy_num'] = $cartTool->getNumWithID($goodsBeans[$i]['goods_id'] . '|' . '0'); // 查询用户该商品的购买量
                ...
                if (!empty($spec_option)) {
                    if ($state == 1 || $state == 2) {
                        ...
                        $spec_option[0]['goods_pv'] = $goodsBeans[$i]['goods_pv'];
                    }
                    foreach ($spec_option as $k2 => $so) {
                        ...
                        if ($k2 > 0) {
                            $spec_option[$k2]['goods_pv'] = $spec_option[$k2]['new_price'] - ($so['goods_price'] - $so['goods_pv']);
                        }
                    }
                    $goodsBeans[$i]['spec_option'] = $spec_option;
...
//                    $goodsBeans[$i]['buy_num'] = $cartTool->getNumWithID($goodsBeans[$i]['goods_id'] . '|' . $goodsBeans[$i]['spec_option'][0]['specs']); // 查询用户该商品的购买量
                ...
                foreach ($goodsBeans[$i]['spec_option'] as $k3 => $gspec) {
                    ...
                    $goodsBeans[$i]['spec_option'][$k3]['goods_pv'] = $goodsBeans[$i]['spec_option'][$k3]['new_price'] - ($gspec['goods_price'] - $gspec['goods_pv']);
                    if ($k3 == 0) {
                        $goodsBeans[$i]['goods_pv'] = $goodsBeans[$i]['spec_option'][$k3]['goods_pv'];
                    }
                }
            ...
            $cartMultiData = $this->getIsCartMultiByGoodsId($goodsBeans[$i]['goods_id']);
            $goodsBeans[$i]['is_cart_multi'] = $cartMultiData['is_cart_multi'];
            if (isset($cartMultiData['gs_id'])) {
                $goodsBeans[$i]['gs_id'] = $cartMultiData['gs_id'];
            }
...
//        $cartTool->saveItem();
    ...
    public function getRecommendPick($pickupList = [])
    {
        ...
        $req = getRequest();
        $lng = empty($req['longitude']) ? $req['lng'] : $req['longitude'];
        $lat = empty($req['latitude']) ? $req['lat'] : $req['latitude'];
        if (!empty($recommendPickups)) {
            if (count($recommendPickups) === 1) {
                return $recommendPickups[0]['id'];
            }
            foreach ($recommendPickups as $key => $pickup) {
                $pickup['distance'] = getPickupDistance($pickup['latitude'], $pickup['longitude'], $lat, $lng);
                $recommendPickups[$key] = $pickup;
                $isKm = strpos($pickup['distance'], 'km') !== false;
                if ($isKm) {
     ...
     * Version: 1.00
     */
    public function getCartData($gsIds = [])
    {
        $options = $this->extra;
        $memberId = $this->memberId;
        // 返回数据结构
        ...
        $cartStoreInfo = D('Store')->getStoreInfo($this->cartStoreId)['data'];
        // 如果是独立店 才处理场景
        if (isAloneMall($cartStoreInfo['store_type'])) {
            $req = getRequest();
            $sessionId = $req['session_id'];
            if ($req['from'] == 'mini') {
                $sessionId = $req['key'];
            }
            $outerStr = S("outer_str:{$this->cartStoreId}:{$sessionId}");
            $outerStrData = S("outer_str_data:{$this->cartStoreId}:{$sessionId}");
            logWrite("购物车场景值:{$sessionId}-{$outerStr}-{$outerStrData}");
        }

        if (pickupIsOpen($this->cartStoreId)) {
            // 配送排序类型 1自提在前
            switch ($this->storeInfo['delivery_type']) {
                case 1:
                    // 如果用户没选择过 默认选择自提
                    if (empty($options)) {
                        $this->setFreightType(1);
                        $options = $this->extra;
                    }
        ...
        // 获取购物车数据
        $goodsList = $this->getShopCartGoodsList($gsIds);
        if (empty($goodsList)) {
            $returnData['checkAll'] = 0;
        ...
        // 获取所有商品可用的自提列表
        $pickupList = [];
        if (pickupIsOpen($this->cartStoreId)) { /*&& $options['freight_type'] == 1*/
...
//                }
        ...
        foreach ($cartGoods as $key => $value) {
            ...
            // 场景值默认
            if ($outerStrData > 0) {
                $this->setPickId($currentStoreId, $outerStrData);
                $options['pickup_ids'][$currentStoreId]['pick_id'] = $outerStrData;
            }
...
//                            $returnData['settleMsg'] = "{{$storeInfo['store_name']}}存在不支持上门自提的商品,无法结算";
        ...
        // region 运费计算；不是自提并且没有配置城际配送 则计算；postage_tag 0-不可下单 1-需运费；postage 运费
        $returnData['freight_data'] = ['is_show' => 0];
        if ($returnData['is_pickup'] == 0) {
            $freightTpl = D('FreightTpl')->field('tpl_id')->where(['store_id' => $this->cartStoreId, 'is_delete' => 0])->find();
            if (!empty($freightTpl)) goto end;
            $distanceData = jsonDecodeToArr($this->storeInfo['distance_info']);
            if (empty($distanceData)) goto end;
            setFreightTipsData($returnData, $this->storeInfo);
        }
        // endregion
        // 这里要更新购物车数据 因为修改过
        end:
     ...
     * @return array
     * User: hjun
     * Date: 2019-03-13 14:17:00
     * Update: 2019-03-13 14:17:00
     * Version: 1.00
     */
    public function getGoodsSpecDataInCart($goodsId = 0)
    {
        $items = $this->items;
        $gsIds = [];
        ...
        $data = [];
        $items = $this->getShopCartGoodsList($gsIds);
        $data['goods_id'] = $items[0]['goods_id'];
        $data['goods_name'] = $items[0]['goods_name'];
        $data['items'] = $items;
        return $data;
    }
     ...
     * @return int
     * User: hjun
     * Date: 2019-03-13 14:23:02
     * Update: 2019-03-13 14:23:02
     * Version: 1.00
     */
    public function getBuyNumByGoodsId($goodsId = 0)
    {
        $items = $this->items;
        $num = 0;
        foreach ($items as $key => $value) {
    ...
    /**
     * 根据商品ID获取商品在购物车是否有多个规格
     * @param int $goodsId
     * @return array
     * User: hjun
     * Date: 2019-03-13 15:42:38
     * Update: 2019-03-13 15:42:38
     * Version: 1.00
     */
    public function getIsCartMultiByGoodsId($goodsId = 0)
    {
        $items = $this->items;
        $gsIds = [];
        foreach ($items as $key => $value) {
        ...
        $data = [];
        $data['is_cart_multi'] = count($gsIds) > 1 ? 1 : 0;
        if (!empty($gsIds)) {
            $data['gs_id'] = $gsIds[0];
        }
        return $data;
