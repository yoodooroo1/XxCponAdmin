...
class OrderLogic extends BaseLogic
{
    ...
    protected $freight_list_one = array('type' => 0, 'name' => '商家配送', 'checked' => 0);
    protected $freight_list_two = array('type' => 1, 'name' => '门店自提', 'checked' => 0);
    protected $istest = 0;
    ...
    public function __construct($storeId, $istest = 0, $name = '', $tablePrefix = '', $connection = '')
    {
        ...
        $this->istest = $istest;
        $this->freight_list_one['name'] = L('KDPS');
        $this->freight_list_two['name'] = L('SMZT');
    }

    public function isAloneMall()
    {
        return isAloneMall($this->storeData['store_type']);
    }

    public function initCart($storeId = 0, $memberId = 0)
    {
        $this->cart = new \Common\Logic\CartLogic($storeId, $memberId);
        $this->cart->getCartData();
    }

    /**
     * 获取确认订单界面数据
     * @param $storeId  这个storeId不一定是公共的store_id
     * @param $memberId
     * @param $passGoodsId  是否传递商品编号  0:没有传递  1已传递
     * @param $goodsIdArray 商品id数组
     * @param $buyCode 二维码字符串
     * @param $addressId 收货地址
     * @param $client //wap[微信移动端]  web[移动端]  ios  android  pc
     * @param $appVersion //版本号
     * @param $latitude  经度
     * @param $longitude  纬度
     * @param $app_name  app名称
     * @param $invoice_id   自提编号
     * @param $is_pickup  是否为自提
     * @param $come_source  来源
     * @return array
     * User: czx
     * Date: 2018/3/18 15:19:45
     * Update: 2018/3/18 15:19:45
     * Version: 1.00
     */
    public function getConfirmOrderData($storeId, $memberId, $passGoodsId, $goodsIdArray, $buyCode,
                                        $addressId, $client, $appVersion, $latitude, $longitude,
                                        $app_name, $invoice_id, $is_pickup, $come_source, $pend_num_id)
    {

        $confirmOrderData = array();
        $confirmOrderData['appVersion'] = $appVersion;
        /***第一步:获取商品数组***/
        $this->cart = new CartLogic($storeId, $memberId);
        $goods_datas = array();
        $store_id_array = array();

        if ($passGoodsId != 1) {
            if (!empty($buyCode)) { //获取扫码的商品id数组
                $returnData = D("QrcodeBuy")->getQrcodeBuyGoodsId($buyCode);
                $goodsIdArray = $returnData['data'];
                if (empty($goodsIdArray)) return getReturn(-1, "请添加扫码商品");
            } else {
                $goodsIdArray = $this->cart->getSelectAllGoodsId();
            }
        } else {
            $goodsIdArray = json_decode($goodsIdArray, true);
        }
        if (empty($goodsIdArray)) return getReturn(-2, "商品数量为0,请添加商品");
        // hjun 2019-03-01 12:17:08 如果是立即购买 初始化购物车
        if (!empty($come_source)) {
            $cartData = $this->cart->getCartData($goodsIdArray)['data'];
        }
        $isAbroad = 0;
...
//            return getReturn(-2, "商品数量为0,请添加商品".$goods_bean['goods_barcode']);
            if (empty($goods_bean))
                continue;
        ...
        /***第三步:获取满减***/
        $this->getStoreMjGoodsList($storeInfoArray);
        ...
        for ($i = 0; $i < count($storeInfoArray); $i++) {
            ...
            $pickup_data = $this->get_store_pickup($storeshopdata_one, $this->channelData,
                $this->mainStoreData['store_id'], $latitude, $longitude);
            $storeInfoArray[$i]['pickUpData'] = $pickup_data;
        ...
        /**第七步:获取运费信息**/
        if (empty($is_pickup) && empty($pend_num_id)) {
            $this->commonOrderApi->getOrderFreight($storeInfoArray, $latitude, $longitude, $addressId);
        } else if (empty($pend_num_id)){
            $this->commonOrderApi->getPickFreight($storeInfoArray, $latitude, $longitude, $addressId);
        }else{
            for ($i = 0; $i < count($storeInfoArray); $i++) {
                $returnData = array();
        ...
        $confirmOrderData['pend_data'] = array();
        if (!empty($pend_num_id)){
            $confirmOrderData['pend_data'] = M("mb_pendingorder_nums")->where(array('id' => $pend_num_id))->find();
        }

        $confirmOrderData['pend_data_list'] = M("mb_pendingorder_nums")->where(array('store_id' => $storeId))->select();
        ...
        for ($i = 0; $i < count($storeInfoArray); $i++) {
           // if (empty($is_pickup)) {
                $storeInfoArray[$i]['total_amount'] = $storeInfoArray[$i]['goods_amount']
                    + $storeInfoArray[$i]['freight']['freight'] - $storeInfoArray[$i]['mj_price'];
//            } else {
//                $storeInfoArray[$i]['total_amount'] = $storeInfoArray[$i]['goods_amount']
//                    - $storeInfoArray[$i]['mj_price'];
//            }
            $storeInfoArray[$i]['temp_total_amount'] = $storeInfoArray[$i]['total_amount'];
            $storeInfoArray[$i]['rest_total_amount'] = $storeInfoArray[$i]['total_amount'];
            $storeshopdata_one = $storeInfoArray[$i];
            $goods_total_num += $storeshopdata_one['goods_total_num'];
...
//            }
        ...
        $confirmOrderData['credits_model'] = 0;
        /**第九步:获取个人可抵用电子币信息**/
        ...
        for ($i = 0; $i < count($storeInfoArray); $i++) {
            ...
            $storeInfoArray[$i]['credits_model'] = $oneStoreData['credits_model'];
            $storeInfoArray[$i]['credits_limit_buy'] = $oneStoreData['credits_limit_buy'];
            ...
            if ($storeInfoArray[$i]['credits_model'] == 1){
                $confirmOrderData['credits_model'] = 1;
                if($storeInfoArray[$i]['credits_limit_buy'] == 1){
                    if ($storeInfoArray[$i]['store_goods_credits'] > $storeInfoArray[$i]['credit_num']){
                        $storeInfoArray[$i]['credits_can_buy'] = 0;
                    }else{
                        $storeInfoArray[$i]['credits_can_buy'] = 1;
                    }
                    $storeInfoArray[$i]['max_credit_num'] = $storeInfoArray[$i]['store_goods_credits'];
                }else{
                    if ($storeInfoArray[$i]['store_goods_credits'] >= $storeInfoArray[$i]['credit_num']){
                        $storeInfoArray[$i]['max_credit_num'] = $storeInfoArray[$i]['credit_num'];
                        $storeInfoArray[$i]['credits_can_buy'] = 1;
                    }else{
                        $storeInfoArray[$i]['credits_can_buy'] = 1;
                        $storeInfoArray[$i]['max_credit_num'] = $storeInfoArray[$i]['store_goods_credits'];
        ...
        if ($this->storeData['channel_type'] == 2 && !empty($this->mainStoreData)) {
            ...
            $storeOneConfigData = M("mb_store_config")->where(array('store_id' => $this->mainStoreData['store_id']))->find();
            ...
            if (empty($storeOneConfigData)) {
                $confirmOrderData['invoice_ctrl'] = 0;
            } else {
                $confirmOrderData['invoice_ctrl'] = $storeOneConfigData['invoice_ctrl'];
            }
            ...
            $storeOneConfigData = M("mb_store_config")->where(array('store_id' => $this->storeData['store_id']))->find();
            if (empty($storeOneConfigData)) {
                $confirmOrderData['invoice_ctrl'] = 0;
            } else {
                $confirmOrderData['invoice_ctrl'] = $storeOneConfigData['invoice_ctrl'];
        ...
        /**无效:获取最大的第三方余额**/
//        $thirdpart_money_most = 0;
//        if (empty($confirmOrderData['storeInfoArray']))
//            $exchange_postage = $confirmOrderData['thirdpart']['exchange_postage'];
...
//                        $thirdpart_money_most = $thirdpart_money_most + $goods_bean['goods_price'] * $goods_bean['buy_num'];
//                    } else {
//                        if ($goods_bean['thirdpart_money_limit'] <= $goods_bean['goods_price']) {
//                            $thirdpart_money_most = $thirdpart_money_most + $goods_bean['thirdpart_money_limit'] * $goods_bean['buy_num'];
//                        } else {
//                            $thirdpart_money_most = $thirdpart_money_most + $goods_bean['goods_price'] * $goods_bean['buy_num'];
//                        }
//                    }
//                }
//                if ($exchange_postage == 1) {  //运费放置前端计算
//                    $thirdpart_money_most += $storeshopdata_one['freight']['freight'];
//                }
//
//            }
//        $confirmOrderData['thirdpart_money_most'] = $thirdpart_money_most;
        $confirmOrderData['storeInfoArray'] = $storeInfoArray;

        /**第十步:获取支付方式**/
        $confirmOrderData['payMethod'] = $this->commonOrderApi->getPayMode($client, $storeId, $app_name, $is_pickup);

        /**第十一步:获取收货地址**/
        $confirmOrderData['addressData'] = D("Address")->getAddressInfo($addressId)['data'];

        /**第十二步:获取发票信息**/
        $defaultInvoice = D('Invoice')->getMemberDefaultInvoice($memberId);
        if (!empty($defaultInvoice)) {
            $confirmOrderData['invoice_data'] = $defaultInvoice;
        } else {
            $confirmOrderData['invoice_data'] = (object)[];
        }

        /**第十三:收货地址列表**/
        if (!empty($come_source)) {
            $this->freight_list = $cartData['freight_list'];
            if (!empty($this->freight_list)) {
                $this->freight_list[0]['checked'] = 0;
                $this->freight_list[1]['checked'] = 0;
                foreach ($this->freight_list as $key =>$value) {
                    if ($is_pickup == 1) {
                        if ($value['type'] == 1) {
                            $this->freight_list[$key]['checked'] = 1;
                        }
                    } else {
                        if ($value['type'] == 0) {
                            $this->freight_list[$key]['checked'] = 1;
        ...
        $confirmOrderData['freight_list'] = $this->freight_list;

        /**第十三:获取自提名称和电话**/
        $memberPickData  = M("member")->where(array('member_id' => $memberId))->find();
        $confirmOrderData['member_pickdata'] = $memberPickData;
        ...
        for ($j = 0; $j < count($confirmOrderData['storeInfoArray']); $j++) {
            if (!empty($confirmOrderData['storeInfoArray'][$j]['storeMemberCoupons']['use'])) {
                ...
                /*******计算end********/
                if ($confirmOrderData['credits_model'] == 1){
                    $storeMaxCouponsId = 0;
                }
                /*******计算2********/
                if ($storeMaxCoupons > 0 && $storeMaxCouponsId > 0) {
                    for ($i = 0; $i < count($confirmOrderData['storeInfoArray'][$j]['storeMemberCoupons']['use']); $i++) {
                        if ($confirmOrderData['storeInfoArray'][$j]['storeMemberCoupons']['use'][$i]['id'] == $storeMaxCouponsId) {
                            ...
                            $restTotalAmount = round($confirmOrderData['storeInfoArray'][$j]['total_amount'] -
                                $confirmOrderData['storeInfoArray'][$j]['storeMemberCoupons']['use'][$i]['coupons_exmoney'], 2);
                            if ($restTotalAmount < 0) {
                                return getReturn(-1, "获取最大优惠券分配有误");
                            }
    ...
    public function getStoreGoodsBean($store_id_array, $goods_datas)
    {
        ...
        foreach ($store_id_array as $key1 => $value1) {
            ...
            $one_data['shop_close'] = $storeOneData['shop_close'];
            $one_data['shop_close_content'] = $storeOneData['shop_close_content'];
            $one_data['balance_exchange_postage'] = 1;
            $storeConfigData = M("mb_store_config")->field('store_id,delivery_switch,delivery_content,custom_pick_switch
            ,custom_pick_type,custom_start_time,custom_end_time')->where(array('store_id' => $value1))->find();
            $one_data['delivery_switch'] = $storeConfigData['delivery_switch'];
            $one_data['delivery_content'] = $storeConfigData['delivery_content'];
            $one_data['custom_pick_switch'] = $storeConfigData['custom_pick_switch'];
            $one_data['custom_pick_type'] = $storeConfigData['custom_pick_type'];
            $one_data['custom_start_time'] = $storeConfigData['custom_start_time'];
            $one_data['custom_end_time'] = $storeConfigData['custom_end_time'];
            $goods_amount = 0;
            $goods_total_num = 0;
            $store_goods_credits = 0;
...
//                    if ($value2['credits_limit'] > ($value2['price'] * $storeOneData['credit_to_money'])){
//                        $store_goods_credits += ($value2['price'] * $storeOneData['credit_to_money'] * $value2['buy_num']);
//                    }else{
                        $store_goods_credits += ($value2['credits_limit'] * $value2['buy_num']);
                  //  }

                }
            }
            $one_data['store_goods_credits'] = $store_goods_credits;
    ...
    {
        //查询某个用户某个商家的优惠券
        $storeInfo = D('Store')->getStoreInfo($storeshopdata_one['store_id'])['data'];
        $condition = array();
        ...
        for ($j = 0; $j < count($membercoupons_datas); $j++) {
            ...
            if ($one_coupons['coupons_type'] == 2) {
                ...
                $one_coupons['coupons_value']['value'] = round($one_coupons['coupons_discount'] * 10, 2);
                $one_coupons['coupons_value']['unit'] = L('FOLD');
            } else {
                $one_coupons['coupons_value']['value'] = $one_coupons['coupons_money'];
                $one_coupons['coupons_value']['unit'] = $storeInfo['currency_unit'];
            }
            $one_coupons['coupons_value']['money'] = "{$one_coupons['coupons_value']['value']}{$one_coupons['coupons_value']['unit']}";
...
//            }
    ...
    public function get_store_pickup($storeshopdata_one, $channel_data, $main_store_id, $latitude, $longitude)
    {
        ...
        $storeIds = [];
        foreach ($store_str as $id) {
            if (!in_array($id, $storeIds) && pickupIsOpen($id)) {
                $storeIds[] = $id;
            }
        }
        $condition = array();
        $condition['store_id'] = getInSearchWhereByArr($storeIds);
        $condition['is_pick'] = 1;
        $condition['type'] = PickUpModel::PICKUP;
        $condition['isdelete'] = 0;
        ...
        foreach ($pickup_datas as $key => $value) {
            ...
            // region hjun 2018-11-09 15:34:57 如果类型是1 才需要查出有库存的门店（与规格对应）
            $storeInfo = D('Store')->getStoreInfo($storeshopdata_one['store_id'])['data'];
            if (pickupIsAlone($storeInfo)) {
                $where = [];
        ...
        if ($is_select == 0 && count($pickup_data_array) > 0) {
            $selectPickId = $this->cart->getRecommendPick($pickup_datas);
            foreach ($pickup_data_array as $key => $value) {
                if ($value['id'] == $selectPickId) {
                    $pickup_data_array[$key]['is_select'] = 1;
                    $is_select = 1;
                } else {
                    $pickup_data_array[$key]['is_select'] = 0;
                }
            }
     ...
     * Version: 1.00
     */
    public function createOrderData($mallstore_order_array, $client, $app_name)
    {

        if ($this->storeData['shop_close'] == 1){
            return getReturn(-1, "店铺已关闭下单:".$this->storeData['shop_close_content']);
        }
        if (empty($this->storeData)) return getReturn(-1, "商家信息为空");
        $returnData = $this->commonOrderApi->checkOrderExit($mallstore_order_array['order_sn']);
        if ($returnData['code'] == 200) {
        ...
        /**第三步:判断订单商品是否正确 团购不检查**/
        if (empty($isGroupOrder)) {
            $returnData = $this->checkOrderData($mallstore_order_array, $this->storeData, $this->mainStoreData);
            if ($returnData['code'] != 200) {
                return $returnData;
            }
        }
        ...
        $returnData = $this->checkMjData($mallstore_order_array);
        if ($returnData['code'] != 200) {
            return $returnData;
        }

        if ($mallstore_order_array['total_price'] == 0){
            $mallstore_order_array['pay_type']  = 3;
        }

        $returnData = $this->checkPayType($client, $mallstore_order_array['merge_store_id'], $app_name, $mallstore_order_array['pay_type'], $mallstore_order_array['is_pickup']);
        if ($returnData['code'] != 200) {
            return $returnData;
        }

        $returnData = $this->checkFreight($mallstore_order_array, $mallstore_order_array['latitude'], $mallstore_order_array['longitude'], $mallstore_order_array['address_id']);
        if ($returnData['code'] != 200) {
            return $returnData;
        }
        ...
        //防止重复下单
        $returnData = D("OrderOnly")->addOrderOnly($mallstore_order_array['order_sn']);
        if ($returnData['code'] != 200) {
            $returndata = D("OrderOnly")->getOrderOnly($mallstore_order_array['order_sn']);
            if (!empty($returndata) && !empty($returndata['data']['return_data'])) {
                return getReturn(200, "返回成功", json_decode($returndata['data']['return_data']));
            } else {
        ...
        try {
            ...
            if ($mallstore_order_array['channel_type'] == 2) {  //商城
                $returnData = D("MergeOrder")->addMergeOrder($mallstore_order_array);
                if ($returnData['code'] != 200) {
                    $this->rollback();
                    return $returnData;
                }
                $morder_id = $returnData['data']['morder_id'];
                $mergepay = 1;
                $mergeorder_id = $returnData['data']['mergeorder_id'];
            } else {   //单店
                $morder_id = 0;
                $mergepay = 0;
                $mergeorder_id = 0;
            ...
            if (!empty($mallstore_order_array['is_pickup']) ) {
                M("member")->where(array('member_id' => $mallstore_order_array['buyer_id']))->save(array('pick_membername' => $mallstore_order_array['order_membername']
                , 'pick_tel' => $mallstore_order_array['order_membertel']));
            }

            if (!empty($mallstore_order_array['idcard']) && !empty($mallstore_order_array['truename'])
                && !empty($mallstore_order_array['address_id'])
            ) {
                $returnData = M("mb_address")->where(array('address_id' => $mallstore_order_array['address_id']))
                    ->save(array('idcard' => $mallstore_order_array['idcard'], 'truename' => $mallstore_order_array['truename']));
                if ($returnData === false) {
                    $this->rollback();
                    return getReturn(-1, "真实身份证信息到地址失败");
                }
            }
            $this->commit();
        } catch (Exception $e) {
        ...
        $orderversion_max = Model("mb_order")->max('version');
        ...
        for ($i = 0; $i < count($storeorder_array); $i++) {
            try {
                ...
                if (round($storeorder['totalprice'] , 2) == 0 && ($storeorder['credits_exmoney'] > 0
                        || $storeorder['coupons_exmoney'] > 0
                        || $storeorder['platform_coupons_money'] > 0
                        || $storeorder['balance'] > 0
                        || $storeorder['platform_balance'] > 0
                        || $storeorder['thirdpart_money'] > 0)
                ) {
                    $storeorder['pay_type'] = 3;
                    $storeorder['pay_name'] = "余额支付";
                } else {
                    $storeorder['pay_type'] = $mallstore_order_array['pay_type'];
                }

                if ($storeorder['pay_type'] == 0) {
                    $storeorder['pay_name'] = "线下付款";
                } else if ($storeorder['pay_type'] == 1) {
                    $storeorder['pay_name'] = "微信支付";
                } else if ($storeorder['pay_type'] == 3) {
                    $storeorder['pay_name'] = '余额支付';
                } else if ($storeorder['pay_type'] == 4) {
                    $storeorder['pay_name'] = '支付宝支付';
                }
                // hjun 2018-03-26 15:57:43 商品根据分类排序
                $goodsList = $storeorder['order_content'];
                $goodsList = array_sort($goodsList, 'gc_id', 'ASC');
                $storeorder['order_content'] = $goodsList;

                $returnData = D("Order")->addOrderData($mallstore_order_array, $storeorder, $client, $orderversion_max);
                ...
                if ($returnData['code'] != 200) {
                    ...
                    if ($returnData['data']['order_state'] == 0 || $returnData['data']['order_state'] == 1){
                        sendOrderMsg($order_id, 0);
                    }
                    if(empty($mallstore_order_array['morder_id'])){
                        $morder_id = $morder_id;
                        $mallstore_order_array['morder_id'] = $morder_id;
                    }
                    ...
                    if (empty($mallstore_order_array['buy_code']) ) {
                        ...
                        for ($p = 0; $p < count($storeorder_array); $p++) {
                            ...
                            if (count($storeorder2['order_content']) <= 50){
                                $result = $input->optionStorageAndSales(-1, $storeorder2['order_content'],
                                    0, 1, $storeorder2['storeid'], $mallstore_order_array['buyer_id'], $order_id);
                                if (!isSuccess($result)) {
                                    $this->rollback();
                                    return getReturn(CODE_ERROR, $result['msg']);
                                }
                            }
                        }
                    }
                    ...
                    if ($storeorder['get_to_store'] > 0 && $storeorder['pickup_id'] > 0) {
                        $returnData = D("PickUpOrder")->addPickUpOrder($storeorder['pickup_id'], $order_id, $mallstore_order_array['buyer_id'],
                            $mallstore_order_array['order_membername'], $all_goods_count,
                            $storeorder['storeid'], $storeorder['get_to_store']);
                        if ($returnData['code'] != 200) {
                            $this->rollback();
                            return $returnData;
                        }
                    ...
                    $returnData = D("OrderGoods")->put_order_goods_db($storeorder['order_content'], $order_id, $storeorder['storeid']);
                    if ($returnData['code'] != 200) {
                        $this->rollback();
                        return $returnData;
                    }

                    $returnData = D("FundTradeDetail")->put_fund_db($data, $order_id, $mallstore_order_array['channel_type']);
                    if ($returnData['code'] != 200) {
                        $this->rollback();
                        return $returnData;
                    }

                    $returnData = D("UnionOrder")->unionorder($order_id, $storeorder['storeid'], $storeid_data);
                    if ($returnData['code'] != 200) {
                        $this->rollback();
                        return $returnData;
                    }
                    ...
                    if ($data['order_state'] == 0) {
                        $send_array = [];
                        $send_array['se'] = $data['storeid'];
                        $send_array['store_id'] = $data['storeid'];
                        $send_array['type'] = 15;
                        $send_array['member_id'] = $data['buyer_id'];
                        $send_array['order_id'] = $order_id;
                        $send_array['is_api'] = 1;
                        $this->commonOrderApi->sendMessage($send_array);
                    }
                    if ($data['order_state'] == 0 || $data['order_state'] == 1 ){
                         sendOrderMsg($order_id, 0);
                    }
                }
            ...
            if ($storeorder['thirdpart_money'] > 0) {
                ...
                if ($pay_result['result'] == -1) {
                    ...
                    Model("mb_order")->where(array('order_id' => $order_id))->update(array('thirdpart_pay' => 1));
                }
            }
        ...
        Model("mb_order_only")->where($where)->update(array('return_data' => json_encode($return_array, 256)));

        foreach ($return_array['mergeorder_content'] as $value) {
            $gs_id_arr = array();
            foreach ($value['order_content'] as $value2) {
                $delete_cart = new CartLogic($value['storeid'], $mallstore_order_array['buyer_id']);
                $gs_id = $value2['gs_id'];
                if (empty($gs_id)) {
            ...
            $delete_cart->delItems($gs_id_arr);
        }
    ...
    public function checkOrderMallBaseData(&$mallstore_order_array, $channelData, $mainStoreData)
    {
        ...
        if (empty($mallstore_order_array['idcard'])) $mallstore_order_array['idcard'] = '';
        if (empty($mallstore_order_array['truename'])) $mallstore_order_array['truename'] = '';
        if (empty($mallstore_order_array['share_member_id'])) $mallstore_order_array['share_member_id'] = '';
        if (empty($mallstore_order_array['buy_code'])) $mallstore_order_array['buy_code'] = '';
        if (empty($mallstore_order_array['order_type'])) $mallstore_order_array['order_type'] = '';
        if (empty($mallstore_order_array['group_id'])) $mallstore_order_array['group_id'] = 0;
        if (empty($mallstore_order_array['latitude'])) $mallstore_order_array['latitude'] = 0;
        if (empty($mallstore_order_array['longitude'])) $mallstore_order_array['longitude'] = 0;
        if (empty($mallstore_order_array['is_pickup'])) $mallstore_order_array['is_pickup'] = 0;
        if (empty($mallstore_order_array['address_id'])) $mallstore_order_array['address_id'] = 0;
        if (empty($mallstore_order_array['isAbroad'])) $mallstore_order_array['isAbroad'] = 0;
        if (empty($mallstore_order_array['is_pickup']) && empty($mallstore_order_array['address_id']) && empty($mallstore_order_array['pend_num_id'])) {
            return getReturn(-4, "请填写收货地址");
        }
        if (!empty($mallstore_order_array['is_pickup']) && (empty($mallstore_order_array['order_membername']) || empty($mallstore_order_array['order_membertel']))) {
            return getReturn(-4, "请填写自提联系人或自提电话号码");
        }
        if (count($mallstore_order_array['mergeorder_content']) <= 0) return getReturn(-5, "订单信息不能为空");
        if (!empty($mallstore_order_array['isAbroad']) && (empty($mallstore_order_array['idcard']) || empty($mallstore_order_array['truename']))) {
            return getReturn(-5, "海外购,请填写身份证信息和真实姓名");
        }
        //额外添加
        $mallstore_order_array['channel_type'] = $channelData['store_type'];
        $mallstore_order_array['channel_id'] = $channelData['channel_id'];
        if (!empty($mallstore_order_array['address_id']) && empty($mallstore_order_array['is_pickup']) && empty($mallstore_order_array['pend_num_id'])) {
            $addressData = D("Address")->getAddressInfo($mallstore_order_array['address_id']);
            if (empty($addressData['data'])) {
                return getReturn(-6, "收货地址编号有误,暂未找到该收货信息");
            } else {
                $mallstore_order_array['addressData'] = $addressData['data'];
                // hjun 2019-06-05 17:45:21 如果开启了验证收货范围 查出主店的配置
                $result = validateDeliveryArea($mallstore_order_array['merge_store_id'], $addressData['data']);
                if (!isSuccess($result)) {
                    return $result;
                }
            }
        } else {
            $mallstore_order_array['addressData'] = array();
        }

        if (!empty($mallstore_order_array['pend_num_id'])){
            $pendData = M("mb_pendingorder_nums")->where(array('id' => $mallstore_order_array['pend_num_id']))->find();
            if (empty($pendData)){
                return getReturn(-6, "点餐桌号不能为空");
            }
        }
        ...
        $confirmData = F("order/{$mallstore_order_array['order_sn']}");
        ...
        for ($i = 0; $i < count($mallstore_order_array['mergeorder_content']); $i++) {
            ...
            for ($j = 0; $j < count($confirmData['storeInfoArray']); $j++) {
                ...
                if ($mallstore_order_array['mergeorder_content'][$i]['storeid'] == $confirmData['storeInfoArray'][$j]['store_id']) {
                    $mallstore_order_array['mergeorder_content'][$i]['credits_model'] = $confirmData['storeInfoArray'][$j]['credits_model'];

                    if (!empty($mallstore_order_array['mergeorder_content'][$i]['pickup_id'])) {
                        $pickupListData = D("PickUp")->where(array('id' => $mallstore_order_array['mergeorder_content'][$i]['pickup_id']))
                            ->find();
                        $mallstore_order_array['mergeorder_content'][$i]['pickUp'] = $pickupListData;
                    } else {
                        $mallstore_order_array['mergeorder_content'][$i]['pickUp'] = array();
                    }
                    ...
                    for ($t = 0; $t < count($orderContent); $t++) {
                        ...
                        $goodsBean['primary_id'] = $goodsData['primary_id'];
                        // hjun 2018-11-12 14:40:53 下单时记录商品的规格状态 方便库存恢复时的判断
                        $goodsBean['spec_open'] = $goodsData['spec_open'];
//                        if($goodsBean['spec_open'] == 1){ //新规格是否开启
...
//                        }
                        ...
                        $goodsBean['goods_qrcode'] = $goodsData['goods_qrcode'];
                        $goodsBean['goods_number'] = $goodsData['goods_number'];
                        $goodsBean['credits_limit'] = $goodsData['credits_limit'];
    ...
    public function checkOrderData(&$mallstore_order_array, $storeData, $mainStoreData)
    {
        ...
        if (empty($mallstore_order_array['buy_code'])) {
            ...
            for ($i = 0; $i < count($storeorder_array); $i++) {
                ...
                $quehuo_array = array_merge($quehuo_array, $input->checkGoods(0,
                    $storeorder['order_content'], $storeorder['storeid'], $mallstore_order_array['buyer_id'], $storeorder));
        ...
        for ($i = 0; $i < count($storeorder_array); $i++) {
            ...
            if ($storeorder['credits_num'] > 0) {
                ...
                if($oneStoreData['credits_model'] == 0){
                    if (round($storeorder['credits_exmoney'],2) > round($storeorder['credits_num'] / $oneStoreData['credit_to_money'], 2)) {
                        return getReturn(-21, '店铺积分抵扣有误');
                    }

                    if ($total_amount < $oneStoreData['credit_limit_money'] && $oneStoreData['credits_model'] == 0) {
                        return getReturn(-22, '订单金额小于积分可使用金额');
                    }

                    if (($storeorder['credits_exmoney'] > $total_amount * $oneStoreData['credit_percent']) && $oneStoreData['credits_model'] == 0) {
        ...
        for ($i = 0; $i < count($storeorder_array); $i++) {
            ...
            if ($storeorder['gou_type'] == 1) {
                ...
                if ($coupons_data['coupons_type'] == 2) {
                    ...
                    $coupons_goods_price = $coupons_goods_price * (1 - $coupons_data['coupons_discount']);
    ...
    public function checkPayType($client, $storeId, $app_name, $pay_type, $is_pickup)
    {

        $payMode = $this->commonOrderApi->getPayMode($client, $storeId, $app_name, $is_pickup);
        if ($pay_type == 0) {
    ...
    public function checkFreight(&$mallstore_order_array, $latitude, $longitude, $addressId)
    {
        ...
        for ($i = 0; $i < count($mallstore_order_array['mergeorder_content']); $i++) {
            ...
            if (empty($storeshopdata_one['pickup_id'])) {
                ...
                $addressData = M("mb_pickup_list")->where(array('id' => $storeshopdata_one['pickup_id']))->find();
                if ($addressData['freight'] == 0){
                    return getReturn(200, "成功");
                }
                $isLocal = $this->commonOrderApi->getLocalValue($storeDataOne, $addressData);
                if ($isLocal == 1) {  //同城方式计算
...
//                    }
    ...
    public function DistributionOrderGoodsData(&$mallstore_order_array)
    {
        ...
        for ($i = 0; $i < count($storeorder_array); $i++) {
            ...
            $use_postage = $storeorder['postage_platform_coupons_money'];
...
//        }
        if ($exchangeType == self::THIRPART_MONEY) {
            if ($storeOrder['thirdpart_money'] > 0) {
                for ($j = 0; $j < count($goods_array); $j++) {
                    $goods_bean = $goods_array[$j];
                    if ($goods_bean['thirdpart_money_limit'] == -1) {
                        $exchangeGoodsPriceSum += $this->commonOrderApi->getGoodsRestPrice($goods_bean);
                    } else {
                        if (($goods_bean['thirdpart_money_limit'] * $goods_bean['gou_num']) > $this->commonOrderApi->getGoodsRestPrice($goods_bean)) {
                            $exchangeGoodsPriceSum += $this->commonOrderApi->getGoodsRestPrice($goods_bean);
                        } else {
            ...
            for ($j = 0; $j < count($goods_array); $j++) {
                $goods_bean = $goods_array[$j];
                if ($storeOrder['thirdpart_money'] > 0) {
                    $oneValue = $this->commonOrderApi->getGoodsRestPrice($goods_bean);
                    if ($goods_bean['thirdpart_money_limit'] != -1) {
            ...
            if ($storeOrder['balance'] > 0) {
                for ($j = 0; $j < count($goods_array); $j++) {
                    ...
                    if ($goods_bean['balance_limit'] == -1) {
                        ...
                        if (($goods_bean['balance_limit'] * $goods_bean['gou_num']) > $this->commonOrderApi->getGoodsRestPrice($goods_bean)) {
                            $exchangeGoodsPriceSum += $this->commonOrderApi->getGoodsRestPrice($goods_bean);
                        } else {
        ...
        }else if($exchangeType == self::CREDITS){
//            if ($storeOrder['credits_exmoney'] > 0) {
//                for ($j = 0; $j < count($goods_array); $j++) {
//                    $goods_bean = $goods_array[$j];
//                    if (($goods_bean['credits_limit'] * $goods_bean['gou_num']) > $this->commonOrderApi->getGoodsRestPrice($goods_bean)) {
//                        $exchangeGoodsPriceSum += $this->commonOrderApi->getGoodsRestPrice($goods_bean);
//                    } else {
//                        $exchangeGoodsPriceSum += $goods_bean['credits_limit'] * $goods_bean['gou_num'];
//                    }
//                }
//            }
//            if ($calulatePostage == 1) {
...
//                if ($storeOrder['credits_exmoney'] > 0) {
//                    $oneValue = $this->commonOrderApi->getGoodsRestPrice($goods_bean);
//                    if (($goods_bean['credits_limit'] * $goods_bean['gou_num']) < $oneValue) {
//                        $oneValue = $goods_bean['credits_limit'] * $goods_bean['gou_num'];
//                    }
//                    $goods_bean[$beanKeyName] = $this->commonOrderApi->getScaleValue($oneValue,
//                        $exchangeGoodsPriceSum, $storeOrder[$storeKeyName], $goods_bean['gou_num']);
//
...
//                    $goods_bean[$beanKeyName] = 0;
//                }
//                $goods_bean['credits_num'] = $goods_bean[$beanKeyName] * $storeOrder['credit_to_money'];
                $goods_bean['credits_num'] = $goods_bean['credits_limit'] * $goods_bean['gou_num'];
                $storeOrder['order_content'][$j] = $goods_bean;
            }
        } else {
            ...
            for ($i = 0; $i < count($storeOrder['order_content']); $i++) {
                ...
                if ($this->checkGoodsUseCoupons($goods_bean, $coupons_data)) {
                    $storeOrder['order_content'][$i]['coupons_exmoney'] = round(($goods_bean['goods_price'] * $goods_bean['gou_num']
                            - $goods_bean['mj_bean_price']) / $coupons_goods_price * $coupons_data['coupons_money'], 4);
    ...
    public function getMaxCouponsConfirm($storeOrder, $coupons_data)
    {
        ...
        if ($coupons_data['coupons_money'] > 0) {
            for ($i = 0; $i < count($storeOrder['order_content']); $i++) {
                ...
                if ($this->checkGoodsUseCoupons($goods_bean, $coupons_data)) {
                    $storeOrder['order_content'][$i]['coupons_exmoney'] = round(($goods_bean['goods_price'] * $goods_bean['gou_num']
                            - $goods_bean['mj_bean_price']) / $coupons_goods_price * $coupons_data['coupons_money'], 2);
                }
        ...
        if ($confirmOrderData['thirdpart']
            ...
            for ($i = 0; $i < count($confirmOrderData['storeInfoArray']); $i++) {
                ...
                for ($j = 0; $j < count($storeshopdata_one['order_content']); $j++) {
                    $goods_bean = $storeshopdata_one['order_content'][$j];
                    if ($goods_bean['thirdpart_money_limit'] == -1) {
                        $store_thirdpart_money_most += $this->commonOrderApi->getGoodsRestPrice($goods_bean);
                    } else {
        ...
        for ($i = 0; $i < count($confirmOrderData['storeInfoArray']); $i++) {
            if ($confirmOrderData['storeInfoArray'][$i]['balancepay'] > 0
                ...
                for ($j = 0; $j < count($storeshopdata_one['order_content']); $j++) {
                    $goods_bean = $storeshopdata_one['order_content'][$j];
                    if ($goods_bean['balance_limit'] == -1) {
                        $most_balance += $this->commonOrderApi->getGoodsRestPrice($goods_bean);
                ...
                if ($most_balance > $confirmOrderData['storeInfoArray'][$i]['balance']) {
                    ...
                    $confirmOrderData['storeInfoArray'][$i]['rm_balance'] = $confirmOrderData['storeInfoArray'][$i]['balance'];

                } else {
                    $confirmOrderData['storeInfoArray'][$i]['most_balance'] = $most_balance;
                    $confirmOrderData['storeInfoArray'][$i]['rm_balance'] = $most_balance;
                }
...
//                + emptyToZero($confirmOrderData['storeInfoArray'][$i]['rm_balance']);
            ...
            if($confirmOrderData['storeInfoArray'][$i]['credits_model'] == 1){
                if ($confirmOrderData['storeInfoArray'][$i]['credits_can_buy'] == 0){
...
//                    for ($j = 0; $j < count($storeshopdata_one['order_content']); $j++) {
//                        $goods_bean = $storeshopdata_one['order_content'][$j];
//                        if ($goods_bean['credits_limit'] <= ($goods_bean['goods_price'] * $confirmOrderData['storeInfoArray'][$i]['credit_to_money']) ) {
//                            $goods_credits_limit = $this->commonOrderApi->getRestPrice($goods_bean) * $confirmOrderData['storeInfoArray'][$i]['credit_to_money'];
//                            if ($goods_credits_limit > $goods_bean['credits_limit'])
//                                $goods_credits_limit = $goods_bean['credits_limit'];
//                            $most_credits += ($goods_credits_limit * $goods_bean['buy_num']);
//                        } else {
//                            $most_credits += ($this->commonOrderApi->getGoodsRestPrice($goods_bean) * $confirmOrderData['storeInfoArray'][$i]['credit_to_money']);
//                        }
//
//                    }
//                    $most_credits = saveDownDecimal($most_credits);
//
//                    if ($most_credits > $confirmOrderData['storeInfoArray'][$i]['credit_num']) {
//                        $exchange_credit_num = $confirmOrderData['storeInfoArray'][$i]['credit_num'];
//                        $exchange_credit_price = saveTwoDecimal($exchange_credit_num / $confirmOrderData['storeInfoArray'][$i]['credit_to_money'], 2);
//
//                        $confirmOrderData['storeInfoArray'][$i]['rm_credit_num'] =
//                            $exchange_credit_num;
//                        $confirmOrderData['storeInfoArray'][$i]['rm_credit_price'] =
//                            $exchange_credit_price;
//
//                    } else {
//                        $exchange_credit_num = $most_credits;
//                        $exchange_credit_price = saveTwoDecimal($most_credits / $confirmOrderData['storeInfoArray'][$i]['credit_to_money'], 2);
//
...
//                        - $confirmOrderData['storeInfoArray'][$i]['rm_credit_price']);
                    ...
                    $confirmOrderData['storeInfoArray'][$i]['rm_credit_num'] = $confirmOrderData['storeInfoArray'][$i]['max_credit_num'];
                    $confirmOrderData['storeInfoArray'][$i]['rm_credit_price'] = 0;
                }
            }else{
...
//            - emptyToZero($confirmOrderData['rm_platform_balance']);
        ...
        if ($confirmOrderData['temp_total_amount'] < 0) {
            return getReturn(-1, "最大抵用金额分配错误1.4");
        }
