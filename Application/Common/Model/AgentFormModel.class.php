 ...
 * User: hjun
 * Date: 2019-04-09 15:04:06
 * Update: 2019-04-09 15:04:06
 * Version: 1.00
 */
class AgentFormModel extends BaseModel
{
    protected $tableName = 'mb_agent_form';
    ...
    */
    public function getFormOrderList($store_id = '', $where = array(), $page = 1, $limit = 15)
    {
        ...
        $where['isdelete'] = 0;
        $start = ($page - 1) * $limit;
        $order_ids = M('mb_agent_form_order')->where($where)->limit($start, $limit)->order('order_id DESC')->getField('order_id', true);
        $lists = array();
        ...
        $result = array();
        $result['code'] = 200;
        $result['data']['total'] = M('mb_agent_form_order')->where($where)->count();
        $result['data']['lists'] = $lists;
        return $result;
    }
    ...
    */
    public function getMemberFormOrderList($store_id = '', $where = array(), $page = 1, $limit = 15)
    {
        ...
        $where['isdelete'] = 0;
        $start = ($page - 1) * $limit;
        $lists = M('mb_agent_form_order')->where($where)->limit($start, $limit)->order('order_id DESC')->select();
        $formInfo = array();
    ...
    */
    ...
    {
        $where = array();
        $where['xunxin_mb_agent_form_order.order_id'] = $order_id;
        $where['xunxin_mb_agent_form_order.store_id'] = $store_id;
        $where['xunxin_mb_agent_form_order.isdelete'] = 0;
        $info = M('mb_agent_form_order')->join('xunxin_mb_form ON xunxin_mb_form.form_id=xunxin_mb_agent_form_order.form_id')->where($where)->field('xunxin_mb_agent_form_order.*,xunxin_mb_form.form_name')->find();
    ...
    /*获取表单提交申请详情
      ...
      * params array $store_id 店铺ID
    */
    public function getMemberFormOrderInfo($formId = '', $member_id = '', $store_id = '')
    {
        $where = array();
        ...
        $where['store_id'] = $store_id;
        $where['isdelete'] = 0;
        $info = M('mb_agent_form_order')->where($where)->field(true)->find();
        if (!empty($info)) {
