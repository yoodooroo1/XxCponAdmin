...
//        die();
     ...
     * @return array
     * User: hjun
     * Date: 2019-06-10 15:13:03
     * Update: 2019-06-10 15:13:03
     * Version: 1.00
     */
    public function getGoodsBeanByGsId($goodsId = 0, $specId = '', $storeId = 0, $memberId = 0)
    {
        if (isSpecIdEmpty($specId)) {
            $specId = 0;
        }
        $goodsBean = $this->getGoodsBean($goodsId, $storeId, $memberId);
        // 获取出规格的原价和PV
        if (!empty($goodsBean['spec_option'])) {
            foreach ($goodsBean['spec_option'] as $spec) {
                if ($spec['specs'] == $specId) {
                    $goodsBean['goods_price'] = $spec['goods_price'];
                    $goodsBean['goods_pv'] = $spec['goods_pv'];
                    break;
                }
            }
        }
...
//        $where['goods_storage'] = ['neq', 0];
     ...
     * @return string
     * User: hjun
     * Date: 2019-04-03 15:34:19
     * Update: 2019-04-03 15:34:19
     * Version: 1.00
     */
    protected function getTableByOpenFrom($storeId = 0, $openFrom = '', $channelId = 0)
    {
        $nowTime = NOW_TIME;
        switch ($openFrom) {
            case 'rush_goods':
                $sWhere = "storeid = {$storeId} AND isdelete = 0 AND islongtime = 0 AND start_time <= {$nowTime} AND end_time > {$nowTime}";
                return M('mb_sales')->field('gid')->where($sWhere)->distinct(true)->select(false);
                break;
            case 'sales_goods':
                $sWhere = "storeid = {$storeId} AND isdelete = 0 AND ( islongtime = 1  OR ( islongtime = 0 AND start_time <= {$nowTime} AND end_time > {$nowTime} ) )";
                return M('mb_sales')->field('gid')->where($sWhere)->distinct(true)->select(false);
                break;
            default:
                // 商城活动
     ...
     * Version: 1.00
     */
    public function getGoodsListByClassId($storeId = 0, $memberId = 0, $gcId = 0, $sortType = 0, $page = 1, $limit = 0, $condition = [], $request = [])
    {
        $suffix = $this->isSortSales($sortType) ? 'a.' : '';
        if (!empty($request['open_from'])) {
            $suffix = 'a.';
        }
        switch ((int)$sortType) {
            case self::SORT_PRICE_ASC:
                $order = "{$suffix}goods_price ASC";
                break;
            case self::SORT_PRICE_DESC:
                $order = "{$suffix}goods_price DESC";
                break;
            ...
            case self::SORT_SALES_ASC:
                ...
                break;
            case self::SORT_NEW_ASC:
                $order = "{$suffix}onshelf_time ASC";
                break;
            case self::SORT_NEW_DESC:
                $order = "{$suffix}onshelf_time DESC";
                break;
            default:
                $order = "{$suffix}top DESC,{$suffix}sort DESC";
                if ($request['from'] == 'mini') {
                    $now = NOW_TIME;
                    $order = "(qianggou_start_time <= {$now} AND qianggou_end_time > {$now} AND is_qianggou = 1) DESC";
                    $order .= ",{$suffix}top DESC,{$suffix}sort DESC";
                }
                break;
        }
        ...
        $join = [];
        $where = [];
        // OPEN_FROM 统一微信的独立商品页面
        if (!empty($request['open_from'])) {
            switch ($request['open_from']) {
                case 'rush_goods':
                    // 限时秒杀
                    $table = $this->getTableByOpenFrom($storeId, $request['open_from']);
                    $join[] = "RIGHT JOIN ({$table}) c ON {$suffix}goods_id = c.gid";
                    break;
                case 'sales_goods':
                    // 手机专享
                    $table = $this->getTableByOpenFrom($storeId, $request['open_from']);
                ...
                case 'new_goods':
                    // 新品推荐 新上架
                    $order = "{$suffix}onshelf_time DESC,top DESC,sort DESC";
                    break;
                case 'tag_goods':
                    break;
                default:
                    // 商城 活动 1 2 4 5 6 7   (3是商家列表 不走这个接口, 8 废弃)
                    $storeInfo = D('Store')->getStoreInfo($storeId)['data'];
                    if (!isMall($storeInfo['store_type'])) {
                        return getReturn(CODE_SUCCESS, 'success', ['list' => [], 'total' => 0, 'currentTotal' => 0]);
                    }
                    if (!in_array($request['open_from'], [1, 2, 4, 5, 6, 7])) {
                        return getReturn(CODE_SUCCESS, 'success', ['list' => [], 'total' => 0, 'currentTotal' => 0]);
                    }
                    $table = $this->getTableByOpenFrom($storeId, $request['open_from'], $storeInfo['channel_id']);
                    $join[] = "RIGHT JOIN ({$table}) e ON {$suffix}goods_id = e.goods_id";
                    $order = 'e.active_sort DESC,e.active_id DESC';
                    $distinct = true;
                    break;
            }
        }
        ...
        if (!empty($request['goods_name'])) {
            $where["{$suffix}goods_name"] = ['LIKE', "%{$request['goods_name']}%"];
        }
        // 是否有货
        if ($request['is_enough'] == 1) {
            $where["{$suffix}goods_storage"] = ['neq', 0];
        }
        // 价格区间
        $priceWhere = getRangeWhere($request, 'min_goods_price', 'max_goods_price')['data'];
        if (!empty($priceWhere)) {
            $where["{$suffix}goods_price"] = $priceWhere;
        }
        // 标签筛选
        $tagIds = explode(',', $request['goods_tag']);
        if (!empty($tagIds)) {
            $activeTagIds = [];
            foreach ($tagIds as $tagId) {
                switch ($tagId) {
                    case TAG_HOT: // 热卖
                        ...
                        break;
                    case TAG_ABROAD: // 境外
                        $where["{$suffix}is_abroad"] = 1;
                        break;
                    case TAG_BOOKING: // 预售
                        $where["{$suffix}is_booking"] = 1;
                        break;
                    case TAG_SPECIAL: // 精选
                        $where["{$suffix}is_special"] = 1;
                        break;
                    case TAG_PROMOTE: // 特价
                        $where["{$suffix}is_promote"] = 1;
                        break;
                    case TAG_QIANGGOU: // 抢购
                        $where["{$suffix}is_qianggou"] = 1;
                        break;
                    default:
                        if ($tagId > 0) {
                            $activeTagIds[] = $tagId;
            ...
            if (!empty($activeTagIds)) {
                $gids = D('GoodsTagLink')->getGoodsIdsByTagsIds($activeTagIds);
                // 如果OPEN_FROM有范围了 需要在此范围内筛选
                if (isset($goodsIds)) {
                    $goodsIds = array_intersect($goodsIds, $gids);
                } else {
                    $goodsIds = $gids;
                }
                $where["{$suffix}goods_id"] = getInSearchWhereByArr($goodsIds);
            }
        }
        ...
        if (!empty($request['keywords'])) {
            $search = [];
            $search["{$suffix}goods_desc"] = getSearchArr($request['keywords']);
            // 条码
            $search["{$suffix}goods_barcode"] = $request['keywords'];
            $search["{$suffix}goods_spec"] = ['like', "\"barcode\":\"{$request['keywords']}\"%"];
            // 多规格组合条码
            $optionWhere = [];
            $optionWhere['goods_barcode'] = $request['keywords'];
            $goodsIds = M('goods_option')
                ->distinct(true)
                ->where($optionWhere)
                ->getField('goods_id', true);
            if (!empty($goodsIds)) {
                $search["{$suffix}goods_id"] = ['in', $goodsIds];
            }
            // 详情
            $search["{$suffix}goods_figure"] = ['like', "%\"text\":\"%{$request['keywords']}%\"%"];
            $search["{$suffix}goods_content"] = ['like', "%{$request['keywords']}%"];
            $search['_logic'] = 'or';
            $where['_complex'] = $search;
        }
        $where = array_merge($where, $condition);
        ...
        if ($this->isSortSales($sortType)) {
            $join[] = 'LEFT JOIN __MB_GOODS_EXP__ b ON a.goods_id = b.goods_id';
        }
        if ($distinct) {
        ...
        $cartTool = new CartLogic($storeId, $memberId);
        $type = 0;
        if ($limit > 0 && in_array($request['open_from'], [2, 6, 7])) {
            // 价格格式
            $type = 2;
        }
        $list = $cartTool->initGoodsBeans($result['data']['list'], $type);
     ...
     * 过期的抢购才需要刷新
     * 一次处理100条
     * type 1-固定 2-每天 3-每周 4-每月
     * @param int $storeId
     * @return array ['code'=>200, 'msg'=>'', 'data'=>null]
     * User: hjun
     * Date: 2019-05-16 18:33:34
     * Update: 2019-05-16 18:33:34
     * Version: 1.00
     */
    public function refreshGoodsQG($storeId = 0)
    {
        $field = [
            'a.goods_id', 'a.qianggou_type', 'a.qianggou_start_time',
            'a.qianggou_end_time', 'a.start_dw', 'a.end_dw',
        ];
        $where = [];
        if (!empty($storeId)) {
            $where['a.store_id'] = $storeId;
        ...
        $where['a.isdelete'] = 0;
        $where['a.is_qianggou'] = 1;
        $where['a.qianggou_type'] = ['gt', 1];
        $where['a.qianggou_end_time'] = ['elt', NOW_TIME];
        $order = 'a.version ASC';
        $page = 1;
        $limit = 500;
        $options = [];
        ...
        if (empty($total)) {
            return getReturn(CODE_SUCCESS, '没有需要刷新的抢购');
        }
        $count = ceil($total / $limit);
        $version = $this->max('version');
        $salesModel = M('mb_sales');
        $saleVersion = $salesModel->max('version');
        try {
            do {
                $options['page'] = $page;
                $options['limit'] = $limit;
                ...
                foreach ($list as $goods) {
                    ...
                    $data['qianggou_start_time'] = $startDate;
                    $data['qianggou_end_time'] = $endDate;
                    $data['goods_id'] = $goods['goods_id'];
                    $data['version'] = ++$version;
                    $this->startTrans();
                    try {
                        $where = [];
                        $where['gid'] = $goods['goods_id'];
                        $where['isdelete'] = 0;
                        // 更新促销表的时间
                        $sale = $salesModel->where($where)->order('sales_id DESC')->find();
                        $saleData = [];
                        $saleData['islongtime'] = 0;
                        $saleData['version'] = ++$saleVersion;
                        $saleData['start_time'] = $data['qianggou_start_time'];
                        $saleData['end_time'] = $data['qianggou_end_time'];
                        if (empty($sale)) {
                            $saleData['gid'] = $goods['goods_id'];
                            $saleData['create_time'] = NOW_TIME;
                            $salesModel->add($saleData);
                        } else {
                            $saleData['sales_id'] = $sale['sales_id'];
                            $salesModel->save($saleData);
                        }
                        // 更新商品表的时间
                        $this->save($data);
                        $this->commit();
            ...
            addExceptionLog($e->getMessage());
            return getReturn(-1, '刷新抢购异常');
        }
        return getReturn(CODE_SUCCESS, '刷新抢购完成');
    }
